...
Текущий demo-frontend и gateway поддерживают `English`, `Chinese` и экспериментальный `Russian`.

Важно: русский включён именно как экспериментальный режим. На stock `GPT-SoVITS-V2` pretrained-весах его качество и стабильность могут быть заметно слабее, чем у `English` и `Chinese`.
...
`gptsovits-gateway` дополнительно создаёт встроенные demo-референсы через `espeak-ng`, включая отдельный русский demo-голос, чтобы режим `ru` можно было проверить сразу после деплоя.
