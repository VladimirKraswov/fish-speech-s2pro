...
- `vllm-omni`
  `text`, `voice`, `reference_id`, `references`, `speed`, `temperature`, `top_p`, `seed`, `language`, `instructions`, `max_new_tokens`, `initial_codec_chunk_frames`
...
- для Fish Speech на `vllm-omni` practical cloning path в этом проекте остаётся прямой `ref_audio/ref_text`
- библиотека reference-ов теперь сохраняет исходный uploaded файл как `source.*`, поэтому cloning может использовать не только нормализованный `sample.wav`, но и исходное более качественное аудио
